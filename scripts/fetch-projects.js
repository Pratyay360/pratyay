#!/usr/bin/env node
/**
 * Fetch markdown posts from GitHub and generate .smd files for Zine.
...
const POSTS_PATH = "content/projects";
...
async function main() {
  ...
  for (const file of mdFiles) {
    ...
    const date = parseDate(frontmatter.date) || extractDateFromSlug(slug);
...
`;
    ...
    const outDir = path.join("content", "blog", slug);
  ...
  console.log(`Done. ${mdFiles.length} posts synced.`);
}
...
function extractDateFromSlug(slug) {
  // Try YYYY-MM-DD prefix in filename
