#!/usr/bin/env node
/**
 * Fetch notes markdown from GitHub and generate .smd files for Zine.
 * Run before `zine release` to sync remote content.
 *
 * Usage: node scripts/fetch-notes.js
 *
 * ── CONFIGURE THESE ──────────────────────────────────
 * Set GITHUB_REPO to the repo containing notes markdown,
 * and NOTES_PATH to the directory inside that repo.
 */
...
const GITHUB_REPO = "blogs_md";       // ← change to your notes repo
const NOTES_PATH = "content/notes";    // ← change to notes path in repo
...
`;
    ...
    const outDir = path.join("content", "notes", slug);
  ...
  console.log(`Done. ${mdFiles.length} notes synced.`);
}
